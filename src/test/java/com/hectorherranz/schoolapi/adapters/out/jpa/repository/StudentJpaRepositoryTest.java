...
class StudentJpaRepositoryTest {
    ...
    @Autowired
    private SpringDataStudentRepository springDataRepository;
    ...
    void shouldSearchStudentsBySchoolIdAndName() {
        ...
        StudentEntity studentEntity3 = new StudentEntity(UUID.randomUUID(), "Bob Johnson", savedSchool2);
        ...
        // When
        Page<Student> results = studentRepository.search(savedSchool1.getId(), "smith", PageRequest.of(0, 10));
    ...
    void shouldSearchStudentsCaseInsensitive() {
        ...
        // When
        Page<Student> results1 = studentRepository.search(savedSchool.getId(), "JOHN", PageRequest.of(0, 10));
        Page<Student> results2 = studentRepository.search(savedSchool.getId(), "smith", PageRequest.of(0, 10));
    ...
    void shouldDeleteStudentById() {
        ...
        // Create student via direct entity creation (simulating School aggregate creation)
        StudentEntity studentEntity = new StudentEntity(UUID.randomUUID(), "Student to Delete", savedSchool);
        StudentEntity savedStudentEntity = springDataRepository.save(studentEntity);
        Student student = StudentEntityMapper.toDomain(savedStudentEntity);
    ...
    void shouldUpdateExistingStudent() {
        ...
        // Create student via direct entity creation (simulating School aggregate creation)
        StudentEntity studentEntity = new StudentEntity(UUID.randomUUID(), "Original Name", savedSchool);
        StudentEntity savedStudentEntity = springDataRepository.save(studentEntity);
        Student originalStudent = StudentEntityMapper.toDomain(savedStudentEntity);

        // When
        Student updatedStudent = new Student(originalStudent.id(), "Updated Name", originalStudent.schoolId());
        Student saved = studentRepository.save(updatedStudent);
    ...
    void shouldHandlePaginationInSearch() {
        ...
        // Create students via direct entity creation (simulating School aggregate creation)
        for (int i = 1; i <= 25; i++) {
            StudentEntity studentEntity = new StudentEntity(UUID.randomUUID(), "Student " + i, savedSchool);
            springDataRepository.save(studentEntity);
        }

        // When
        Page<Student> firstPage = studentRepository.search(savedSchool.getId(), "Student", PageRequest.of(0, 10));
        Page<Student> secondPage = studentRepository.search(savedSchool.getId(), "Student", PageRequest.of(1, 10));
    ...
    void shouldReturnEmptyPageWhenNoStudentsInSchool() {
        ...
        // When
        Page<Student> results = studentRepository.search(savedSchool.getId(), "any", PageRequest.of(0, 10));
