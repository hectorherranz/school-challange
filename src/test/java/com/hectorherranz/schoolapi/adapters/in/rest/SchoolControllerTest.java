...
class SchoolControllerTest {
    ...
    void givenValidRequest_whenUpdateSchool_thenReturns200() {
        ...
        UpdateSchoolCommand expectedCommand = new UpdateSchoolCommand(schoolId, 
    ...
    void givenValidQuery_whenSearchSchools_thenReturnsPagedResponse() {
        ...
        School school2 = School.rehydrate(schoolId2, "Beauxbatons Academy", new Capacity(300), List.of());
        ...
        PagedResponse<School> pagedResponse = new PagedResponse<>(
                List.of(school1, school2), 0, 20, 2, 1, false, false
        );
        ...
        // Act
        ResponseEntity<PagedResponse<SchoolSummary>> response = controller.searchSchools("School", 0, 20);
    ...
    void givenEmptyQuery_whenSearchSchools_thenReturnsAllSchools() {
        ...
        PagedResponse<School> pagedResponse = new PagedResponse<>(
                List.of(school), 0, 20, 1, 1, false, false
        );
    ...
    void givenPagedResults_whenSearchSchools_thenReturnsCorrectPagingFlags() {
        ...
        PagedResponse<School> pagedResponse = new PagedResponse<>(
                List.of(school1, school2), 0, 20, 40, 2, true, false
        );
    ...
    void givenOverCapacity_whenCreateSchool_thenThrowsCapacityExceededException() {
        ...
        when(schoolDtoMapper.toCreateCommand(request)).thenReturn(expectedCommand);
        when(createSchoolUseCase.handle(expectedCommand)).thenThrow(new CapacityExceededException(UUID.randomUUID()));
    ...
    void givenNonExistentId_whenUpdateSchool_thenThrowsNotFoundException() {
        ...
        UUID nonExistentId = UUID.randomUUID();
        SchoolRequest request = new SchoolRequest("Updated School Name", 600);
        UpdateSchoolCommand expectedCommand = new UpdateSchoolCommand(nonExistentId, 
                java.util.Optional.of("Updated School Name"), 
                java.util.Optional.of(600));

        when(schoolDtoMapper.toUpdateCommand(nonExistentId, request)).thenReturn(expectedCommand);
        doThrow(new NotFoundException("School not found")).when(updateSchoolUseCase).handle(expectedCommand);
