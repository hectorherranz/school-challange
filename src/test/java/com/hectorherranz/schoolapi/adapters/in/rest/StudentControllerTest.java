...
class StudentControllerTest {
    ...
    void givenValidQuery_whenSearchStudents_thenReturnsPagedResponse() {
        ...
        PagedResponse<Student> pagedResponse = new PagedResponse<>(
                List.of(student1, student2), 0, 20, 2, 1, false, false
        );
        ...
        // Act
        ResponseEntity<PagedResponse<StudentResponse>> response = controller.searchStudents(schoolId, "Harry", 0, 20);
    ...
    void givenEmptyQuery_whenSearchStudents_thenReturnsAllStudents() {
        ...
        PagedResponse<Student> pagedResponse = new PagedResponse<>(
                List.of(student), 0, 20, 1, 1, false, false
        );
        ...
        // Act
        ResponseEntity<PagedResponse<StudentResponse>> response = controller.searchStudents(schoolId, "", 0, 20);
    ...
    void givenPagedResults_whenSearchStudents_thenReturnsCorrectPagingFlags() {
        ...
        PagedResponse<Student> pagedResponse = new PagedResponse<>(
                List.of(student1, student2), 0, 20, 40, 2, true, false
        );
    ...
    void givenOverCapacity_whenCreateStudent_thenThrowsCapacityExceededException() {
        ...
        when(studentDtoMapper.toCreateCommand(request)).thenReturn(expectedCommand);
        when(createStudentUseCase.handle(expectedCommand)).thenThrow(new CapacityExceededException(schoolId));
    ...
    void givenNonExistentId_whenUpdateStudent_thenThrowsNotFoundException() {
        ...
        doThrow(new NotFoundException("Student not found")).when(updateStudentUseCase).handle(expectedCommand);
    ...
    void givenNonExistentSchoolId_whenCreateStudent_thenThrowsNotFoundException() {
        ...
        CreateStudentCommand expectedCommand = new CreateStudentCommand("Harry Potter", nonExistentSchoolId);

        when(studentDtoMapper.toCreateCommand(request)).thenReturn(expectedCommand);
        when(createStudentUseCase.handle(expectedCommand)).thenThrow(new NotFoundException("School not found"));
